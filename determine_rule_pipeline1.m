function [rules ,weights] = determine_rule_pipeline1(input, output, individual,weights_original)
...
for i = 1:length(input)  %%算每个属性 只要有一个属性沾边就算一个激活权重 最后合成 这就是并集 而交集只有都沾边才算激活权重
   ...
   if i ==1
    ...
    % Dynamic adjustment for minority class 激活程度加权
        if output == 1  % minority class
            weights_activated = weights_activated *2;  % Amplify weights for minority class
        end
